...
{
    public class TurnoffmeterModel : PageModel
    {
        enum StateEnum
        {
            ready,
        ...
        static string truckIdentification = "Device number " + truckNum;
        ...
        //static string ScopeID = "0ne00164785";
        //static string DeviceID = "SmartMeterv1.1v3-1";
        //static string PrimaryKey = "KdV0TuyiF0ltz334HGKl/86T3ThAowfSNVLUuQbuzLM=";
        static string AzureMapsKey = "2Dy8XjlS8NWk1YYpR1kLn8iZvPXGaKM4EmnGRG9mKpM";
        ...
        public TurnoffmeterModel(IBillingRepository BillingData, IDeviceRepository DeviceData)
        {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            try
            {
                ...
                Message = "Failed to register device";
                greenMessage("Device Disconnected successfully ");
        ...
        static void UpdateTruck()
        {
            ...
            else
            {
                ...
                if (fan == FanEnum.on)
                {
                    tempContents += -0.2 + DieRoll(3);
                }
                else
                {
                    tempContents += -0.5 + DieRoll(2);
                }
        ...
        async void SendTruckTelemetryAsync(Random rand, CancellationToken token)
        {
            while (true)
            {
                ...
                {
                    EnergyAmountkWh = Math.Round(tempContents, 1),
                    VoltageReading = Math.Round(tempContents, 2),
                    PowerReading = Math.Round(tempContents, 3),
                    ConnectionStatus = 2,
        ...
        {
            reportedProperties["DeviceID"] = truckIdentification;
